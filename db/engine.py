...
unreachable -- we deliberately do **not** fall back to JSON or SQLite,
because mixing a stateful auth surface with a best-effort store hides bugs.
"""
...
def init_db() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not connect to the database at {_scrub(DATABASE_URL)}.  "
            f"Start Postgres with `docker compose up -d postgres`.  Error: {exc}"
        ) from exc
